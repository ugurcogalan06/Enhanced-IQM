...
class MAE(torch.nn.Module):
    def __init__(self):
        ...
        model_path = os.path.abspath(os.path.join('weights', 'MAE.pth'))
        self.load_state_dict(torch.load(model_path, map_location='cpu'), strict=False)


    def forward(self, y, x, as_loss=True, resize = True):

        score = ((torch.abs(x - y))).mean() 
        
        return score 
    
    
    def MAE_map(self, y, x):
        
        return self.scaler_network(torch.abs(y - x).mean([1], keepdim=True)) - self.scaler_network(torch.tensor(0.0).cuda().reshape(1,1,1,1))
...
class E_MAE(torch.nn.Module):
    def __init__(self):
        ...
        model_path = os.path.abspath(os.path.join('weights', 'E_MAE.pth'))
        self.load_state_dict(torch.load(model_path, map_location='cpu'), strict=False)
...
if __name__ == '__main__':
    ...
    model_emae = E_MAE().to(device)
    model_mae = MAE().to(device)

    E_MAE_err, E_MAE_mask = model_emae.E_MAE_map(dist, ref)
    MAE_err = model_mae.MAE_map(dist, ref)
    
    E_MAE_err = map_visualization(E_MAE_err)
    MAE_err = map_visualization(MAE_err)
    ...
    # plt.show()
    E_MAE_mask = E_MAE_mask.detach().squeeze().cpu().numpy()
    
    os.makedirs(args.save_dir, exist_ok=True)
    save_image(os.path.join(args.save_dir, 'E_MAE_map.png'), E_MAE_err)
    save_image(os.path.join(args.save_dir, 'MAE_map.png'), MAE_err)
    save_image(os.path.join(args.save_dir, 'Mask.png'), E_MAE_mask)
